...
def _write_poscar(atoms, path: str) -> None:
    # sort=True groups atoms by element so the species line is "Mg O"
    # with counts "4 4" instead of "Mg O Mg O Mg O Mg O" / "1 1 1 1 1 1 1 1"
    # — multi-element cells from ase.build.bulk() come in interleaved
    # and break VASP's POSCAR↔POTCAR consistency check otherwise.
    # See validation/BUG_NOTES.md (MgO panel failure, 2026-05-14).
...
def _write_vasp_sbatch(system, rundir: str) -> None:
    """Per-cell SLURM script: load the MPI/MKL toolchain, assemble
    POTCAR, run vasp_std.
    ...
    live on the cluster, not something the agent should emit). The
    element list is baked in from ``system.elements`` — the harness
    already knows it, so there's no need to re-parse POSCAR line 6.

    Module versions mirror the working bcc_fe VASP job on Deception;
    edit them if the cluster toolchain differs.
    """
...
# ── MPI / MKL toolchain (edit versions to match your cluster) ──
module purge
...
# $VASP_PP_PATH points at the POTCAR root, e.g. .../potpaw_PBE.54
